...
| | To get the Taints on master node | <code>kubectl describe nodes master &#124; grep -i taints </code> |
...
| |  To change the image of nginx deployment to 1.9.0 | `kubectl create deployment my-dep --image=nginx` <br> `kubectl edit deployment my-dep` <br> <code>kubectl describe deployment my-dep &#124; grep -i image </code>
...
|--recursive | Print the fields of fields | <code>kubectl explain pod --recursive &#124; less</code> |
...
| | To find option which we need for all namespaces | <code>kubectl get --help &#124; grep namespaces</code> |
| | To find what is the argument for no-headers in kubernetes commands | <code> kubectl get --help &#124; grep -i header </code>
...
| | Set a deployment's nginx container image to `nginx:1.9.1` | `kubectl create deployment my-dep --image=nginx` <br> `kubectl set image deployment my-dep nginx=nginx:1.9.1` <br> <code>kubectl describe deployment my-dep &#124; grep -i image </code>
...
| | Remove from node 'foo' the taint with key 'dedicated' and effect 'NoSchedule' if one exists. <br> Put a `-` at the end of Taint (which you can get using <code> kubectl describe node master &#124; grep -i taint </code> | `kubectl taint nodes foo dedicated:NoSchedule-`
